    ...
    <header class="beem-hero" id="overview"><div class="beem-blob beem-blob-a"></div><div class="beem-blob beem-blob-b"></div>
      <div class="container beem-hero-grid">
        ...
        <div class="beem-hero-visual beem-reveal-scale">
          ...
          <?php beem360_device((string)beem360_options()['media']['hero_primary'],'System KPIs',(string)beem360_options()['media']['hero_secondary']); ?>
        </div>
      </div>
    </header><?php });
}
...
function beem360_problem_shortcode(): string {
    ...
    <section class="beem-section" id="problem"><div class="container"><div class="beem-section-head text-center beem-reveal"><div class="beem-num"><b>01</b> <?php echo esc_html(beem360_x('PROBLEM','المشكلة','PROBLÈME')); ?></div><h2><?php echo esc_html(beem360_t('problem_title')); ?></h2><p><?php echo esc_html(beem360_t('problem_text')); ?></p></div>
      <div class="beem-compare"><article class="beem-compare-card is-bad beem-reveal"><h3><i class="bi bi-x-circle"></i> <?php echo esc_html(beem360_x('WITHOUT BEEM 360','بدون BEEM 360','SANS BEEM 360')); ?></h3><?php foreach($bad as $row){?><div><i class="bi bi-x"></i><b><?php echo esc_html($row['label']??''); ?></b><small><?php echo esc_html($row['meta']??''); ?></small></div><?php } ?></article>
      <div class="beem-compare-arrow"><span><i class="bi bi-arrow-right"></i></span><small>ONE SYSTEM</small></div>
      <article class="beem-compare-card is-good beem-spot beem-reveal"><h3 class="beem-mini-brand"><img src="<?php echo beem360_asset('10-brand-mark.png'); ?>" alt=""> Beem <em>360</em></h3><?php foreach($good as $row){?><div><i class="bi bi-check"></i><b><?php echo esc_html($row['label']??''); ?></b><small><?php echo esc_html($row['meta']??''); ?></small></div><?php } ?></article></div>
    </div></section><?php });
}
...
function beem360_solution_shortcode(): string {
    ...
    <section class="beem-section beem-solution" id="workflow"><div class="container"><div class="beem-section-head text-center beem-reveal"><div class="beem-num"><b>02</b> <?php echo esc_html(beem360_x('SOLUTION','الحل','SOLUTION')); ?></div><h2><?php echo esc_html(beem360_t('solution_title')); ?></h2><p><?php echo esc_html(beem360_t('solution_text')); ?></p></div>
      <div class="beem-hub beem-reveal"><div class="beem-hub-panel"><?php foreach($left as $item){?><div><i class="bi <?php echo esc_attr($item['icon']??'bi-circle'); ?>"></i><?php echo esc_html($item['label']??''); ?></div><?php } ?></div><div class="beem-hub-core"><img src="<?php echo beem360_asset('11-brand-mark.png'); ?>" alt=""><b>Beem <em>360</em></b></div><div class="beem-hub-panel is-right"><?php foreach($right as $item){?><div><i class="bi <?php echo esc_attr($item['icon']??'bi-circle'); ?>"></i><?php echo esc_html($item['label']??''); ?></div><?php } ?></div></div>
    </div></section><?php });
}
...
function beem360_features_shortcode(): string {
    ...
    <section class="beem-section beem-features" id="features"><div class="container"><div class="beem-section-head text-center beem-reveal"><div class="beem-num"><b>03</b> <?php echo esc_html(beem360_x('FEATURES','المزايا','FONCTIONNALITÉS')); ?></div><h2><?php echo esc_html(beem360_t('features_title')); ?></h2><p><?php echo esc_html(beem360_t('features_text')); ?></p></div>
      <?php foreach($features as $i=>$f){$points=preg_split('/\R+/',(string)($f['bullets']??''),-1,PREG_SPLIT_NO_EMPTY);?><article class="beem-feature <?php echo $i%2 ? 'is-flipped' : ''; ?>" id="<?php echo esc_attr($f['id']??'feature-'.$i); ?>"><div class="beem-feature-copy beem-reveal"><span class="beem-feature-tag <?php echo esc_attr($f['color']??'blue'); ?>"><i class="bi <?php echo esc_attr($f['icon']??'bi-stars'); ?>"></i> <?php echo esc_html($f['tag']??''); ?></span><h3><?php echo esc_html($f['title']??''); ?></h3><p><?php echo esc_html($f['text']??''); ?></p><ul><?php foreach($points as $point){?><li><i class="bi bi-check2"></i><?php echo esc_html($point); ?></li><?php } ?></ul><?php if(!empty($f['url'])){?><a class="beem-feature-more" href="<?php echo esc_url($f['url']); ?>"><?php echo esc_html(beem360_x('Learn more','اعرف المزيد','En savoir plus')); ?> <i class="bi bi-arrow-right"></i></a><?php } ?></div><div class="beem-feature-visual beem-reveal-scale"><?php if(($f['image']??'')==='workspace'){beem360_workspace_mock();} else {beem360_device((string)($f['image']??''),(string)($f['tag']??''),(string)($f['image_secondary']??''));} ?></div></article><?php } ?>
    </div></section><?php });
}
