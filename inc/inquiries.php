...
function beem360_submit_inquiry(): void {
    ...
    $subject=sprintf('[Beem View 360] %s from %s', $type==='request'?'Demo request':'Contact message', $name);
...
function beem360_thankyou_email(string $name): string {
    ...
    return '<!doctype html><html><body style="margin:0;background:#f3f7f9;font-family:Arial,sans-serif;color:#132433"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f7f9;padding:32px 14px"><tr><td align="center"><table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 18px 55px rgba(16,41,59,.12)"><tr><td style="height:8px;background:linear-gradient(90deg,#1e96be,#5ab4a0,#faaa3c)"></td></tr><tr><td style="padding:34px 38px 15px"><table role="presentation"><tr><td><img src="'.esc_url($logo).'" width="48" height="48" alt="Beem View 360" style="display:block"></td><td style="padding-left:12px;font-size:21px;font-weight:800;color:#1e96be">Beem View <span style="color:#faaa3c">360</span></td></tr></table></td></tr><tr><td style="padding:14px 38px 38px"><p style="margin:0 0 12px;color:#5a6a78">'.esc_html(beem360_x('Hello','مرحبًا','Bonjour')).' '.esc_html($name).',</p><h1 style="font-size:30px;line-height:1.2;margin:0 0 16px;color:#132433">'.esc_html($title).'</h1><p style="font-size:16px;line-height:1.75;color:#5a6a78;margin:0 0 26px">'.esc_html($text).'</p><a href="'.esc_url($register).'" style="display:inline-block;background:#1e96be;color:#fff;text-decoration:none;font-weight:700;padding:14px 24px;border-radius:11px">'.esc_html(beem360_x('Explore Beem View 360','استكشف Beem View 360','Découvrir Beem View 360')).'</a></td></tr><tr><td style="background:#10293b;color:rgba(255,255,255,.65);padding:18px 38px;font-size:12px">© '.esc_html(wp_date('Y')).' Beem View 360</td></tr></table></td></tr></table></body></html>';
}
