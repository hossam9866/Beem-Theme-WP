(()=>{'use strict';document.documentElement.classList.add('beem-motion');
...
const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;const animated=document.querySelectorAll('.beem-reveal,.beem-reveal-scale');if(!reduced&&'IntersectionObserver'in window){const io=new IntersectionObserver(entries=>entries.forEach(entry=>{if(!entry.isIntersecting)return;const el=entry.target;const peers=[...el.parentElement.children].filter(item=>item.matches?.('.beem-reveal,.beem-reveal-scale'));const index=Math.max(0,peers.indexOf(el));el.style.setProperty('--beem-delay',`${Math.min(index*90,270)}ms`);requestAnimationFrame(()=>el.classList.add('is-in'));io.unobserve(el)}),{threshold:.08,rootMargin:'0px 0px -7% 0px'});animated.forEach(el=>io.observe(el))}else animated.forEach(el=>el.classList.add('is-in'));
if(matchMedia('(hover:hover) and (pointer:fine)').matches)document.querySelectorAll('.beem-spot').forEach(el=>el.addEventListener('pointermove',event=>{const box=el.getBoundingClientRect();el.style.setProperty('--mx',`${(event.clientX-box.left)/box.width*100}%`);el.style.setProperty('--my',`${(event.clientY-box.top)/box.height*100}%`)}));
document.querySelectorAll('[data-count]').forEach(el=>{const target=Number(el.dataset.count);if(reduced){el.textContent=String(target);return;}let start;const run=time=>{start??=time;const p=Math.min(1,(time-start)/1400);el.textContent=String(Math.round(target*(1-Math.pow(1-p,3))));if(p<1)requestAnimationFrame(run)};requestAnimationFrame(run)});
